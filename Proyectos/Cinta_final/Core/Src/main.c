...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
//definicion estados y sub_estados
typedef enum
{
	E_desactivado = 	0,
	E_configurando = 	1,
...
typedef enum
{
	...
	C_pesando =				1,
	C_detenida =			2,
	C_esperando_reinicio =	3,
	C_acelerando =			4,
	C_desacelerando =		5,
	C_esperando_maestro =	6
} SUB_ESTADO;
/* USER CODE END PTD */
...
#define PWM_C_andando 600 //pwm para cinta andando
#define PWM_C_detenido 250 //pwm para el cual la cinta se detiene

#define PULSE_MIN       550 // servo 0°
#define PULSE_MAX       2450 //servp 180°
#define PULSE_REPOSO    1500 //servo deja pasar objeto
...
typedef struct //estructura para definir cada clasificacion, servo=objeto
{
    ...
    int32_t peso_min; //rango de clasificacion
    int32_t peso_max;

    GPIO_TypeDef *sensor_port; //sensor de deteccion asociado
    uint16_t sensor_pin;
    volatile uint8_t sensor_pendiente;
    volatile uint32_t tick_sensor;
    uint32_t ultimo_sensor;

    FIFO cola; //cola de objetos a clasificar

    TIM_HandleTypeDef *timer; // timmer que envia señales al servo
...
static uint8_t aviso_esperando_pendiente = 0;
static uint16_t aviso_esperando_secuencia = 0;

static uint8_t aviso_peso_pendiente = 0;
static uint16_t aviso_peso_decigramos = 0;
...
void Clasificadores_Init(void)
{
    ...
    for (uint8_t i = 0; i < CANTIDAD_CLASIFICADORES; i++)
    {
    	// inicio antirebote, asi procesa desde el inicio
...
static void cinta_generar_aviso_peso(int32_t peso_mg)
{
    ...
    if (peso_mg <= 0){
        ...
        peso_decigramos = ((uint32_t)peso_mg + 50) / 100;
    }

    if (peso_decigramos > 65535){
